...
from .utils import factorize, refactor
...
def combine(dataFrame):
    ...
    all["ExterQual"] = dataFrame["ExterQual"].map(qual_dict).astype(float)
    all["ExterCond"] = dataFrame["ExterCond"].map(qual_dict).astype(float)
    all["BsmtQual"] = dataFrame["BsmtQual"].map(qual_dict).astype(float)
    all["BsmtCond"] = dataFrame["BsmtCond"].map(qual_dict).astype(float)
    all["HeatingQC"] = dataFrame["HeatingQC"].map(qual_dict).astype(float)
    all["KitchenQual"] = dataFrame["KitchenQual"].map(qual_dict).astype(float)
    all["FireplaceQu"] = dataFrame["FireplaceQu"].map(qual_dict).astype(float)
    all["GarageQual"] = dataFrame["GarageQual"].map(qual_dict).astype(float)
    all["GarageCond"] = dataFrame["GarageCond"].map(qual_dict).astype(float)

    all["BsmtExposure"] = dataFrame["BsmtExposure"].map({None: 0, "No": 1, "Mn": 2, "Av": 3, "Gd": 4}).astype(float)

    bsmt_fin_dict = {None: 0, "Unf": 1, "LwQ": 2, "Rec": 3, "BLQ": 4, "ALQ": 5, "GLQ": 6}
    all["BsmtFinType1"] = dataFrame["BsmtFinType1"].map(bsmt_fin_dict).astype(float)
    all["BsmtFinType2"] = dataFrame["BsmtFinType2"].map(bsmt_fin_dict).astype(float)

    all["Functional"] = dataFrame["Functional"].map({None: 0, "Sal": 1, "Sev": 2, "Maj2": 3, "Maj1": 4, "Mod": 5, "Min2": 6, "Min1": 7, "Typ": 8}).astype(float)

    all["GarageFinish"] = dataFrame["GarageFinish"].map({None: 0, "Unf": 1, "RFn": 2, "Fin": 3}).astype(float)

    all["Fence"] = dataFrame["Fence"].map({None: 0, "MnWw": 1, "GdWo": 2, "MnPrv": 3, "GdPrv": 4}).astype(float)
    ...
    all["PoolQC"] = dataFrame["PoolQC"].map(qual_dict).astype(float)
    ...
    all["HighSeason"] = dataFrame["MoSold"].replace({1: 0, 2: 0, 3: 0, 4: 1, 5: 1, 6: 1, 7: 1, 8: 0, 9: 0, 10: 0, 11: 0, 12: 0})

    all["NewerDwelling"] = dataFrame["MSSubClass"].replace({20: 1, 30: 0, 40: 0, 45: 0, 50: 0, 60: 1, 70: 0, 75: 0, 80: 0, 85: 0, 90: 0, 120: 1, 150: 0, 160: 0, 180: 0, 190: 0})
    ...
    all["SaleCondition_PriceDown"] = dataFrame.SaleCondition.replace({'Abnorml': 1, 'Alloca': 1, 'AdjLand': 1, 'Family': 1, 'Normal': 0, 'Partial': 0})

    #* House completed before sale or not
    all["BoughtOffPlan"] = dataFrame.SaleCondition.replace({"Abnorml": 0, "Alloca": 0, "AdjLand": 0, "Family": 0, "Normal": 0, "Partial": 1})

    all["BadHeating"] = dataFrame.HeatingQC.replace({'Ex': 0, 'Gd': 0, 'TA': 0, 'Fa': 1, 'Po': 1})
    ...
    all["SeasonSold"] = all["MoSold"].map({12: 0, 1: 0, 2: 0, 3: 1, 4: 1, 5: 1, 6: 2, 7: 2, 8: 2, 9: 3, 10: 3, 11: 3}).astype(int)

    all["YearsSinceRemodel"] = all["YrSold"] - all["YearRemodAdd"]

    #* Simplifications of existing features into bad/average/good.
    all["SimplOverallQual"] = all.OverallQual.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2, 6: 2, 7: 3, 8: 3, 9: 3, 10: 3})
    all["SimplOverallCond"] = all.OverallCond.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2, 6: 2, 7: 3, 8: 3, 9: 3, 10: 3})
    all["SimplPoolQC"] = all.PoolQC.replace({1: 1, 2: 1, 3: 2, 4: 2})
    all["SimplGarageCond"] = all.GarageCond.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplGarageQual"] = all.GarageQual.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplFireplaceQu"] = all.FireplaceQu.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplFireplaceQu"] = all.FireplaceQu.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplFunctional"] = all.Functional.replace({1: 1, 2: 1, 3: 2, 4: 2, 5: 3, 6: 3, 7: 3, 8: 4})
    all["SimplKitchenQual"] = all.KitchenQual.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplHeatingQC"] = all.HeatingQC.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplBsmtFinType1"] = all.BsmtFinType1.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2, 6: 2})
    all["SimplBsmtFinType2"] = all.BsmtFinType2.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2, 6: 2})
    all["SimplBsmtCond"] = all.BsmtCond.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplBsmtQual"] = all.BsmtQual.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplExterCond"] = all.ExterCond.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
    all["SimplExterQual"] = all.ExterQual.replace({1: 1, 2: 1, 3: 1, 4: 2, 5: 2})
...
def refactored_data_frames():
    """
    @desc: Make the final trim on the dataset and return the normalized
        data for training and testing
    @return: Tuple containing (combined_train, combined_test, label)
    """
    ...
    #* Return the combined_train_data, combined_test_data, label
    return (combined_train, combined_test, label)